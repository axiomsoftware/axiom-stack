 ...
 *
 * $RCSfile: WrappedNodeManager.java,v $
 * $Author: hannes $
 * $Revision: 1.22 $
 ...
 * The idea behind this is that we don't care a lot about Exception classes, since Axiom programming is done
...
public final class WrappedNodeManager {
    ...
    public Node getNode(String id, DbMapping dbmap) {
        axiom.framework.core.RequestEvaluator reqev = nmgr.app.getCurrentRequestEvaluator();
        int mode = DbKey.LIVE_LAYER;
        if (reqev != null) {
            mode = reqev.getLayer();
        }
        return getNode(new DbKey(dbmap, id, mode));
    }
    ...
    public Node getNode(Key key) {
        try {
            ...
            nmgr.app.logError(ErrorReporter.errorMsg(this.getClass(), "getNode") 
            		+ "Error retrieving Node for " + key, x);
    ...
    public Node getNode(Node home, String id, Relation rel) {
        try {
            ...
            nmgr.app.logError(ErrorReporter.errorMsg(this.getClass(), "getNode") 
            		+ "Error retrieving Node \"" + id + "\" from " + home, x);
    ...
    /**
     * @see axiom.objectmodel.db.NodeManager#updateSubnodeList(Node, Relation)
     */
    ...
    public Key getChildKey(String field, String key, String parentid, int mode) {
        try {
            return this.nmgr.getChildKey(field, key, parentid, mode);
        } catch (Exception ex) {
            return null;
        }
    }
