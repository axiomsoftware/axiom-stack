...
JSDOC.JsDoc = function(/**object*/ opt) {
	...
	// added for custom Axiom Stack application documentation
	// Thomas Mayfield - 6/11/2008
	if(JSDOC.opt.i){
		// we need to provide the constructors that axiom provides implicitly
		var buffer = [];
		for each(f in new File(JSDOC.opt._).list()){
			if (f.match(/^\.[^\.\/\\]|static/)) continue; // skip dot files
			var file = new File(JSDOC.opt._ + SYS.slash + f);
			if(file.isDirectory() && file.list().indexOf("prototype.properties") != 0){
				buffer.push("/**");
				buffer.push(" * @constructor");
				buffer.push(" */");
				buffer.push(f+" = function(){}");
			}
		}
		IO.saveFile(JSDOC.opt._, 'all-axiom-prototypes.js', buffer.join("\n"));
	}
	// end Axiom customization
