...
JSDOC.Symbol.prototype.setTags = function() {
	...
	*/
	...
	else if(JSDOC.opt.i && !(/all-axiom-prototypes.js/).test(JSDOC.Symbol.srcFile)){
		var pathdata = JSDOC.Symbol.srcFile.match(/([^\\\/]+)[\\\/][^\\\/]+$/);
		var protoname = pathdata ? pathdata[1]: false;
		if(protoname){
 			this.memberOf = protoname+'#';
		}
	}
